...
// â”€â”€ HTML template helpers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€ Traceability Report â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
<div class="page">
  <div class="header">
    <div class="header-left">
      ...
      <div style="font-size:12px;color:#64748b;margin-top:4px">Full Installation History â€” ${spool.spoolId}</div>
    </div>
  ...
  ${openNCRs.length > 0 ? `<div class="ncr-banner">âš  ${openNCRs.length} Open NCR(s) â€” ${openNCRs.map(n => n.ncrNumber).join(', ')}</div>` : ''}

  <div class="meta">
    <div class="meta-item"><label>Spool ID</label><value>${spool.spoolId}</value></div>
    <div class="meta-item"><label>Line Number</label><value>${spool.line?.lineNumber ?? 'â€”'}</value></div>
    <div class="meta-item"><label>Area</label><value>${spool.line?.area ?? 'â€”'}</value></div>
    <div class="meta-item"><label>Status</label><value>${spool.status.replace(/_/g, ' ')}</value></div>
    <div class="meta-item"><label>Material</label><value>${spool.material ?? 'â€”'}</value></div>
    <div class="meta-item"><label>Size</label><value>${spool.size ?? 'â€”'}</value></div>
    <div class="meta-item"><label>RFID Tag 1</label><value>${spool.rfidTag1 ?? 'â€”'}</value></div>
    <div class="meta-item"><label>Drawing Rev</label><value>${spool.drawing?.revision ?? 'â€”'}</value></div>
  </div>
  ...
  <table>
    ...
    <tr>
      <td>${spool.receivedAt ? new Date(spool.receivedAt).toLocaleDateString() : 'â€”'}</td>
      <td>${spool.receivedBy ?? 'â€”'}</td>
      <td>${[spool.storageZone, spool.storageRack, spool.storageRow, spool.storagePosition].filter(Boolean).join('-') || 'â€”'}</td>
      <td>${spool.issuedTo ?? 'â€”'}</td>
      <td>${spool.issuedAt ? new Date(spool.issuedAt).toLocaleDateString() : 'â€”'}</td>
    </tr>
  </table>
  ...
  <table>
    ...
    <tr>
      ...
      <td>${j.holdFlag ? '<span class="badge badge-yellow">âš  HOLD</span>' : 'â€”'}</td>
  ...
  <table>
    ...
    <tr>
      <td style="font-weight:700">${j.jointId}</td>
      <td>${w.weldDate ? new Date(w.weldDate).toLocaleDateString() : 'â€”'}</td>
      <td>${w.welderId ?? 'â€”'}</td>
      <td>${w.wps ?? 'â€”'}</td>
      <td>${w.weldProcess ?? 'â€”'}</td>
  ...
  <table>
    ...
    <tr>
      ...
      <td>${n.holdFlag ? '<span class="badge badge-yellow">HOLD</span>' : 'â€”'}</td>
    </tr>`)).join('')}
  </table>` : ''}
  ...
  <table>
    ...
    <tr>
      <td>${pt.testDate ? new Date(pt.testDate).toLocaleDateString() : 'â€”'}</td>
      <td>${pt.testType}</td>
      <td>${pt.testPressure ? `${pt.testPressure} bar` : 'â€”'}</td>
      <td>${pt.holdTime ? `${pt.holdTime} min` : 'â€”'}</td>
      <td>${pt.testMedium ?? 'â€”'}</td>
  ...
  <div style="display:flex;gap:20px;flex-wrap:wrap;margin-top:10px">
    ${['QC_INSPECTOR', 'QA_MANAGER', 'CLIENT_INSPECTOR'].map(role => {
      ...
      return `<div class="signature-box">
        ...
        <div style="font-size:9px;color:#94a3b8">${app ? `${app.status} Â· ${app.approvedAt ? new Date(app.approvedAt).toLocaleDateString() : ''}` : 'Pending'}</div>
      </div>`;
    }).join('')}
  </div>

  <div class="footer">
    <span>ParTraceflow MES â€” Pipe Spool System</span>
    <span>Generated: ${new Date().toISOString()}</span>
    <span>Spool: ${spool.spoolId}</span>
  </div>
  <div class="confidential">CONFIDENTIAL â€” For authorized personnel only</div>
</div>
</body></html>`;
}

// â”€â”€ NDE Report â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
<div class="page">
  ...
  ${holds > 0 ? `<div class="hold-banner">âš  ${holds} records on HOLD â€” re-test or disposition required</div>` : ''}
  ...
  <table>
    ...
    ${records.map(r => `
    <tr>
      <td style="font-weight:700">${r.joint?.jointId ?? 'â€”'}</td>
      <td>${r.joint?.spool?.spoolId ?? 'â€”'}</td>
      <td><span class="badge badge-blue">${r.ndeType}</span></td>
      <td>${r.ndeNumber ?? 'â€”'}</td>
      <td>${r.inspector ?? 'â€”'}</td>
      <td>${statusBadge(r.result)}</td>
      <td>${r.holdFlag ? '<span class="badge badge-yellow">HOLD</span>' : 'â€”'}</td>
      <td>${r.createdAt ? new Date(r.createdAt).toLocaleDateString() : 'â€”'}</td>
    </tr>`).join('')}
  </table>

  <div class="footer">
    <span>ParTraceflow MES â€” NDE Report</span>
    <span>Generated: ${new Date().toISOString()}</span>
  </div>
</div></body></html>`;
}

// â”€â”€ Weld Summary Report â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
<div class="page">
  ...
  <table>
    ...
    ${joints.map(j => {
      ...
      return `<tr>
        <td style="font-weight:700">${j.jointId}</td>
        <td>${j.spool?.spoolId ?? 'â€”'}</td>
        <td>${j.jointType.replace('_', ' ')}</td>
        <td>${statusBadge(j.status)}</td>
        <td>${weld?.welderId ?? 'â€”'}</td>
        <td>${weld?.wps ?? 'â€”'}</td>
        <td>${weld?.weldProcess ?? 'â€”'}</td>
        <td>${j.ndeRecords.length}</td>
        <td>${j.holdFlag ? '<span class="badge badge-yellow">âš  HOLD</span>' : 'â€”'}</td>
      </tr>`;
    }).join('')}
  ...
  <div class="footer">
    <span>ParTraceflow MES â€” Weld Summary</span>
    <span>Generated: ${new Date().toISOString()}</span>
  </div>
</div></body></html>`;
}

// â”€â”€ Pressure Test Certificate â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
<div class="page">
  ...
  <div style="border:1px solid #e2e8f0;border-radius:8px;padding:16px;margin-bottom:20px">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:12px">
      <div>
        <div style="font-size:15px;font-weight:700">${pt.spool?.spoolId ?? 'â€”'}</div>
        <div style="font-size:11px;color:#64748b">${pt.spool?.line?.lineNumber ?? 'â€”'} Â· ${pt.spool?.line?.area ?? 'â€”'}</div>
      </div>
      <div>${statusBadge(pt.result ?? 'PENDING')}</div>
    </div>
    <div class="meta">
      <div class="meta-item"><label>Test Type</label><value>${pt.testType}</value></div>
      <div class="meta-item"><label>Test Pressure</label><value>${pt.testPressure ? `${pt.testPressure} bar` : 'â€”'}</value></div>
      <div class="meta-item"><label>Hold Time</label><value>${pt.holdTime ? `${pt.holdTime} min` : 'â€”'}</value></div>
      <div class="meta-item"><label>Test Medium</label><value>${pt.testMedium ?? 'â€”'}</value></div>
      <div class="meta-item"><label>Test Date</label><value>${pt.testDate ? new Date(pt.testDate).toLocaleDateString() : 'â€”'}</value></div>
      <div class="meta-item"><label>Tested By</label><value>${pt.testedBy ?? 'â€”'}</value></div>
    </div>
  ...
  <div class="footer">
    <span>ParTraceflow MES â€” Pressure Test Certificate</span>
    <span>Generated: ${new Date().toISOString()}</span>
  </div>
</div></body></html>`;
}
...
export async function GET(req: NextRequest) {
  ...
  // MEDIUM fix: each request launches a Puppeteer browser to rasterize a
  // PDF with no size/pagination limit on the underlying report queries —
  // previously any of the 4 SPOOL_ROLES (including OPERATOR) could trigger
  // this repeatedly with no throttling, a real CPU/memory DoS vector.
  const ip = getClientIp(req);
  const rl = await rateLimit(`pdf:${ip}`, { limit: 10, windowMs: 60_000 });
  if (!rl.allowed) {
    return NextResponse.json({ error: 'Rate limit exceeded — PDF generation is expensive, please wait before retrying' }, { status: 429, headers: { 'Retry-After': String(Math.ceil(rl.retryAfterMs / 1000)) } });
  }
